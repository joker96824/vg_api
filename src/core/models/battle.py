...
class Battle(Base):
    ...
    start_time = Column(DateTime(timezone=True), default=utc_now, comment="开始时间")
    ...
    # 约束：确保一个房间只能有一个未结束的对战
    __table_args__ = (
        # 唯一约束：房间ID + 非结束状态，确保一个房间只能有一个进行中的对战
        # 注意：这个约束需要数据库支持部分唯一索引
        # 在PostgreSQL中可以使用条件唯一索引
        # CREATE UNIQUE INDEX idx_room_active_battle ON battles (room_id) WHERE status != 'finished' AND is_deleted = false;
    )
